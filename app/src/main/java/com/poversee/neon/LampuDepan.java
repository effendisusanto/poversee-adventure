...
public class LampuDepan extends AppCompatActivity {
    ...
    public LampuDepan lampuDepan = null;
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            try {
                                ...
                                Toast.makeText(lampuDepan, "io exception", Toast.LENGTH_LONG).show();
                            } catch (InterruptedException interuptedException) {
                                Toast.makeText(lampuDepan, "interuptedException", Toast.LENGTH_LONG).show();
                            }
                        }
                    }).start();
                } else {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            Connection conn = Connection.connect(opts);
                            conn.publish("trihatmaja.ctrl.req", "{\"point_number\":2,\"dimm_value\":\""+ finalProgress +"\"}");
                            conn.close();
                        } catch (IOException ioException) {
                            Toast.makeText(lampuDepan, "io exception", Toast.LENGTH_LONG).show();
                        } catch (InterruptedException interuptedException) {
